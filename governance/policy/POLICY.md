...
## Enforcement (Phase 2 — no longer prose)

These rules are executed, not just documented. The gatekeeper `governance/validation/index.js`
runs the validators below between stages; the orchestrator refuses to advance on any failure.

| Rule in this policy | Enforced by |
|---|---|
| Requirement must be testable + cited | `governance/validation/requirement.js` (gate 01) |
| Every AC covered · no orphan cases · negative/boundary present | `coverage.js` (gate 05) |
| Business rules (idempotent create, no literal secrets, expected present) | `business-rules.js` (gate 05) |
| No duplicate cases | `dedup.js` (gate 05) |
| Behavioral assertions only | `assertions.js` (gate 05) |
| Verified, cited, non-positional locators | `locators.js` (gate 07) |
| No networkidle / blind timeout / positional selector in specs | `automation-review.js` (gate 07) |
| **Provenance required** (no fact without a source) | `result.js` → `checkProvenance()` |

Run locally: `npm run test:gates` (self-tests) · `npm run gate <stage> <ctx.json>` (a real gate).
A failing gate exits non-zero, so CI blocks the run.
