...
test.describe.serial('Motadata AIOps Discovery Flow For Linux Server Discovery', () => {
  ...
  test('Create Discovery for Linux Server', async () => {
    ...
    await expect(page.getByRole('gridcell', { name: process.env.Sybase_linux_ip })).toBeVisible();
...
await page.locator('#start-rediscovery').click();
  ...
  await page.locator("//input[@id='instance-jdbc']").fill("master");
  await page.locator('#create-credential-btn-id').click();
  ...
  await expect(page.getByText('Sybase')).toBeVisible({ timeout: 60000 });
  ...
  await page.locator('i.anticon-close').click();
