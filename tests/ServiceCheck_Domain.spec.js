...
test.describe.serial('Motadata AIOps Discovery Flow For Service check Domain', () => {
  let page;
  ...
  test('Create Discovery for Service check Domain', async () => {
    await page.getByText('Service Check', { exact: true }).click();
    await page.locator('input[name="profile-name"]').fill('google.com');
    ...
    await expect(page.getByText("domainservicecheck").first()).toBeVisible({ timeout: 480000 });
