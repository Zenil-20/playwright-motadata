//Please note that the code for Anamoly and Forecast is commented out because clickhouse doesn't support the anomaly and forecast functions. Once clickhouse supports these functions, we can uncomment the code and run the tests for Anamoly and Forecast.
...
// Linux server monitor IP — sourced from .env (Linux_server_172_16_15_132)
const LINUX_SERVER_IP = process.env.Linux_server_172_16_15_132;
...
test.describe.serial('Motadata AIOps Metric Explorer Compare Data verification', () => {
  ...
  test('Instance_KPI_Compare_Metric_Explore_Screen', async () => {
    ...
    await page.locator('//input[@id="assign-monitor-search"]').fill(LINUX_SERVER_IP);

    const ipRow = page.locator('tr.k-master-row').filter({ hasText: LINUX_SERVER_IP });
