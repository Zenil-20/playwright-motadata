...
// The dummy SNMP device for Extreme runs on a non-standard UDP port (1612) and on a
// SECOND IP of the host (so Motadata treats it as a distinct monitor from Ruijie,
// which shares the host). Credentials use the shared "Default SNMP" profile
// (community v2c public) selected from the picker.
const EXTREME_IP = process.env.Extreme_Wireless_IP || '172.16.15.161';
const EXTREME_PORT = process.env.Extreme_Wireless_Port || '1612';
...
test.describe.serial('Motadata AIOps Discovery Flow For Extreme Wireless Discovery', () => {
  let page;
  ...
  test('Create Discovery for Extreme Wireless', async () => {
    ...
    await page.getByRole('link', { name: 'Extreme Wireless' }).click();

    // Profile name must be UNIQUE (backend rejects duplicates) — append a timestamp.
    await page.locator('input[name="profile-name"]').fill(`extreme-wireless-${istStamp()}`);
    ...
    await page.locator('input[name="wireless-ip-address"]').fill(EXTREME_IP);
    ...
    await page.locator('input[name="port"]').fill(EXTREME_PORT);
    ...
    await expect(page.getByText(EXTREME_IP).first()).toBeVisible({ timeout: 120000 });
