...
test.describe.serial('Motadata AIOps Discovery Flow For 172.16.10.1 for v2c creds', () => {
  ...
  test('Create Discovery for Network 172.16.10.1 for v2c creds', async () => {
    await page.getByText('Network', { exact: true }).click();
    await page.locator('input[name="profile-name"]').fill('test discovery for snmp');
    await page.locator("//input[@id='ip-address-id']").fill(process.env.Network_Device_172_16_10_1);
    await page.locator("//button[@id='create-credential-btn-id']").click();
    await page.locator("//input[@id='credential-profile-name-id']").fill("testcredentialprofileforsmpv2c");
    await page.locator("//div[@id='version-id']//input[@placeholder='Select']").click();
    await page.getByRole('menuitem', { name: 'V2c' }).click();
    await page.locator("//input[@id='community-id']").fill('Mtdt@#789');
    ...
    const deviceName = process.env.Network_Device_172_16_10_1;
