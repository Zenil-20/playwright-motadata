...
test.describe.serial('Motadata AIOps Discovery Flow For Service check port Discovery', () => {
  let page;
  ...
  test('Create Discovery for Service check Port', async () => {
    await page.getByText('Service Check', { exact: true }).click();
    await page.locator('input[name="profile-name"]').fill('172.16.8.165port');
    await page.locator('#service-type-id').click();
    await page.locator("//input[@data-cy='dropdown-search-input']").fill('port');
    await page.locator("//span[normalize-space()='Port']").click();
    await page.locator("//span[normalize-space()='IP/Host']").click();
    await page.locator("//input[@id='ip-address-id']").fill("172.16.8.165");
    await page.locator('#save-run-btn-id').click();
    await expect(page.getByText("172.16.8.165").first()).toBeVisible({ timeout: 480000 });
