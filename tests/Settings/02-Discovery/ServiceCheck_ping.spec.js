...
test.describe.serial('Motadata AIOps Discovery Flow For Service check Ping', () => {
  let page;
  ...
  test('Create Discovery for Service check Ping', async () => {
    await page.getByText('Service Check', { exact: true }).click();
    await page.locator('input[name="profile-name"]').fill('172.16.10.134ping');
    await page.locator("//span[normalize-space()='IP/Host']").click();
    await page.locator("//input[@id='ip-address-id']").fill("172.16.10.1");
    await page.locator('#save-run-btn-id').click();
    await expect(page.getByText("172.16.10.134ping").first()).toBeVisible({ timeout: 480000 });
