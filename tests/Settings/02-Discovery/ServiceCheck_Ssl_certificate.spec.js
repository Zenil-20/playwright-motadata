...
test.describe.serial('Motadata AIOps Discovery Flow For Service check ssl certificate Discovery', () => {
  let page;
  ...
  test('Create Discovery for Service check ssl', async () => {
    await page.getByText('Service Check', { exact: true }).click();
    await page.locator('input[name="profile-name"]').fill('https://www.youtube.com');
    await page.locator('#service-type-id').click();
    await page.locator("//input[@data-cy='dropdown-search-input']").fill('ssl');
    await page.locator("//span[normalize-space()='SSL Certificate']").click();
    await page.locator("//span[normalize-space()='IP/Host']").click();
    await page.locator("//input[@id='ip-address-id']").fill("https://www.youtube.com");
    await page.locator('#save-run-btn-id').click();
    await expect(page.getByText("https://www.youtube.com").first()).toBeVisible({ timeout: 480000 });
