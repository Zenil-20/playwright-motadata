...
test.describe.serial('Motadata ObserveOps APM Trace Analytics Policy creation', () => {
  ...
  test('Go to APM Policy and create a new Trace Analytics policy', async () => {
    ...
    // Policy name
    const policyName = `Trace Span Errors Playwright Policy ${runTag()}`;
    ...
    await selectApmCounter(page, 'service.trace.span.errors');
    ...
    // notification (div.notification-header ... 'Warning') can't collide (strict-mode).
    await page.locator('#main-content-container').getByText('Warning', { exact: true }).click();
