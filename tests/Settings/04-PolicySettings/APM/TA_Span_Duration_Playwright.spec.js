...
test.describe.serial('Motadata ObserveOps APM Trace Analytics Policy creation', () => {
  ...
  test('Go to APM Policy and create a new Trace Analytics policy', async () => {
    ...
    // Policy name
    await page.locator('input#policy-name').fill('Span Duration Playwright Policy');
    ...
    await page.locator("//input[@placeholder='Search']").fill('service.span.duration.us');
    await page.getByText('service.span.duration.us', { exact: true }).click();
    ...
    await expect(
      page.locator('td', { hasText: 'Span Duration Playwright Policy' })
    ).toBeVisible();
  });
});
