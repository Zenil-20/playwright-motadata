...
// The five RUM services to register. `title` is the exact option title in the
// Application Type dropdown; `slug` is the artifact filename.
const RUM_APPS = [
  { title: 'Vue',     slug: 'vue',     name: 'Playwright Vue Application' },
  { title: 'React',   slug: 'react',   name: 'Playwright React Application' },
  { title: 'JS',      slug: 'js',      name: 'Playwright JS Application' },
...
test.describe.serial('Motadata AIOps — Register RUM Application for all 5 services (Nginx)', () => {
  ...
  for (const app of RUM_APPS) {
    test(`Register ${app.title} application and store config`, async () => {
      ...
      await expect(drawer.getByRole('heading', { name: 'Register Application' })).toBeVisible();

      // Application Name
      await page.locator('input#rum-application-name-id').fill(app.name);

      // Application Type (custom searchable dropdown -> floating menu)
      await page.locator('[data-cy="dropdown-trigger-input"]').first().click();
      await page.locator(`.virtual-scrollable-dropdown-menu span[title="${app.title}"]`).last().click();

      // Deployment = Nginx is the default selected toggle — no action needed.
      ...
      // Environment is a free-text field.
      ...
      // Submit
      await page.locator('#rum-application-submit-btn').click();
