...
test.describe.serial('Motadata AIOps Discovery Flow For Service check NTP', () => {
  let page;
  ...
  test('Create Discovery for Service check NTP', async () => {
    await page.getByText('Service Check', { exact: true }).click();
    await page.locator('input[name="profile-name"]').fill('time.google.com');
    await page.locator('#service-type-id').click();
    await page.locator("//span[normalize-space()='NTP']").click();
    await page.locator("//span[normalize-space()='IP/Host']").click();
    await page.locator("//input[@id='ip-address-id']").fill("time.google.com");
    await page.locator('#save-run-btn-id').click();
    await expect(page.getByText("time.google.com").first()).toBeVisible({ timeout: 480000 });
